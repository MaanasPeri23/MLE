    ...
    * When `out`, `a`, `b` are stride-aligned, avoid indexing
    ...
    def _zip(
        ...
        if (
            ...
            or (a_strides != b_strides).any()
            or (a_shape != b_shape).any()
            or (b_strides != out_strides).any()
            or (b_shape != out_shape).any()
        ):
            for i in prange(len(out)):
                out_idx = out_shape.copy()
                a_idx = a_shape.copy()
    ...
    for x in prange(out_shape[0]):
        for y in range(out_shape[1]):
            for z in range(out_shape[2]):
                val = 0.0
                a_pos = x * a_batch_stride + y * a_strides[1]
                b_pos = x * b_batch_stride + z * b_strides[2]
