...
class CNNSentimentKim(minitorch.Module):
    ...
    def __init__(
        ...
        # TODO: Implement for Task 4.5.
        #not using self.feature_map_size because this instance is being referenced somewhere else, the above stuff is redundant
        self.oneDConv1 = Conv1d(embedding_size, feature_map_size, filter_sizes[0])
        self.oneDConv2 = Conv1d(embedding_size, feature_map_size, filter_sizes[1])
        self.oneDConv3 = Conv1d(embedding_size, feature_map_size, filter_sizes[2])

        self.lastLinear = Linear(feature_map_size, 1)
        #passing on dropout
    ...
    def forward(self, embeddings):
        ...
        """
        # TODO: Implement for Task 4.5.
        activate = self.oneDConv1.forward(embeddings).relu()
        activate2 = self.oneDConv2.forward(embeddings).relu()
        activate3 = self.oneDConv3.forward(embeddings).relu()

        applyMax = minitorch.nn.max(activate, 2)
        applyMax2 = minitorch.nn.max(activate2, 2)
        applyMax3 = minitorch.nn.max(activate3, 2)

        summed = applyMax + applyMax2 + applyMax3

        #reshaping dimensions
        reshape = self.lastLinear(len(summed), len(summed[0]))
        appliedDropout = minitorch.nn.dropout(reshape, self.dropout) #idk if dropout will change from 0.25 that's why this
        final = appliedDropout.sigmoid()
        finalView = final.view(len(embeddings))
