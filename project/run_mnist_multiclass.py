...
class Network(minitorch.Module):
    ...
    def __init__(self):
        ...
        # TODO: Implement for Task 4.5.
        self.firstConv = Conv2d(1, 4, 3, 3)
        self.secondConv = Conv2d(4, 8, 3, 3)
        self.firstLinear = Linear(392, 64)
        self.secondLinear = Linear(64, 10)
        # raise NotImplementedError("Need to implement for Task 4.5")

    def forward(self, x):
        # TODO: Implement for Task 4.5.
        self.mid = self.firstConv.forward(x).relu()
        self.out = self.secondConv.forward(self.mid).relu()

        #2d pooling
        kernel_size = (4, 4)
        pool = minitorch.avgpool2d(self.out, kernel_size)
        flatten = pool.view(BATCH, 392)
        linear = self.firstLinear.forward(flatten).relu()

        if self.training: 
            #apply dropout only when training
            linear = minitorch.dropout(linear, 0.25)
        
        temp = self.secondLinear.forward(linear)
        logs = minitorch.logsoftmax(temp, dim=1)
        return logs
        # raise NotImplementedError("Need to implement for Task 4.5")
