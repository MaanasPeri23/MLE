...
def assert_close(a, b, eps=1e-5):
    assert abs(a - b) < eps, f"Expected {a} to be close to {b}"
...
def test_max(t: Tensor) -> None:
    ...
    assert_close(out[0, 0, 0], max([t[0, 0, x] for x in range(4)]))
    out = minitorch.nn.max(t, 1)
    assert_close(out[0, 0, 0], max([t[0, y, 0] for y in range(3)]))
    out = minitorch.nn.max(t, 0)
    assert_close(out[0, 0, 0], max([t[z, 0, 0] for z in range(2)]))
